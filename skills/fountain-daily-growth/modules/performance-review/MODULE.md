...
own baseline, reads the operator's decisions from the post metadata, and writes what holds beyond today
...
4. Read the operator's decisions from `meta.status` and the timestamps.
   A draft approved fast, edited before approval, or left untouched each says something.
   The operator's edits to title, text, or context are the closest thing to a reason - diff them.
